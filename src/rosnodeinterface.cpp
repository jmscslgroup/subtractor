...
// C/C++ source code generated on : Mon Nov 06 21:59:05 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"subtractor\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(subtractor_M, (NULL));
        subtractor_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(subtractor_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(subtractor_M)
                == (NULL)) || rtmGetStopRequested(subtractor_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(subtractor_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
